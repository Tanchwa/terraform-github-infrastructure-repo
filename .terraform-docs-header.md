...
Variables related to the authentication of the cloud provider will be pre-pended with their ususal prefix for that context. For example, ARM_, AWS_, GOOGLE_. Additionally, any non ARM\_ variables for Azure will be pre-pended with AZ\_, such as AZ\_STATE\_STORE.

Any OTHER variables specific to the repo instance will NOT be pre-pended. Some examples of these variables are: PROJECT_ID (GCP) and RESOURCE_GROUP_NAME (Azure).
